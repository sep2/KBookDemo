...
object KBook extends LazyLogging {
  ...
  def apply(config: KBookConfig): KBook = {
    ...
    val linearized = config.dag.dag
      .ensuring(_.isAcyclic,
                throw new RuntimeException("The graph is not acyclic"))
      .ensuring(_.isDirected,
                throw new RuntimeException("The graph is directed"))
      .ensuring(_.isConnected,
                throw new RuntimeException("The graph is connected"))
      .topologicalSort
      .right
      .getOrElse(throw new RuntimeException("The graph has no node"))
    ...
    linearized.foldLeft(List[BaseNode]())((constructed, current) => {
      val inbounds = BaseNode.SchemaBounds(
        current.incoming.toSeq
          ...
          .foldLeft(Seq[BaseNode.SchemaBound]()) { (ibs, x) =>
            ibs :+ x._1
              .asInstanceOf[logicnode.Predecessor]
              .outbound(x._2.fromPort)
          })
      ...
      val newNode: BaseNode = nodeConfig.meta.`type` match {
        case Dag.NodeType.Source =>
          Source(clazz, Source.Environment(ctx, nodeConfig))
        case Dag.NodeType.Transformer =>
          Transformer(clazz, Transformer.Environment(ctx, nodeConfig, inbounds))
        case Dag.NodeType.Sink =>
          Sink(clazz, Sink.Environment(ctx, nodeConfig, inbounds))
      }
